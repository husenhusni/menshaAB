...
import { StyleSheet, View, Text, ScrollView, Image, TouchableOpacity, Alert, Modal, Pressable, FlatList, LogBox, } from "react-native";
    ...
    <View style={styles.onGoingCard}>
      <View style={styles.mainContent}>
        ...
        <View style={styles.textContainer}>
          <Text numberOfLines={1} style={{ ...styles.nameText }}>{item.transactionData.first_name} {item.transactionData.last_name}</Text>
          <Text style={styles.statusText}>{item.transactionData.status}</Text></View>
        <View style={styles.numberContainer}>

          <Text style={{ ...styles.nameText }}>{item.transactionData.amountReceiving} birr</Text>
          <Text style={{ ...styles.timeText}}>{item.transactionData.data}</Text>
        </View>
      </View>
      <Divider />
      <View style={{ flexDirection: "row", display: "flex", alignSelf: "stretch" }}>
        <Text numberOfLines={2} style={{ ...styles.regularText, marginLeft: 15, flex: 0.76 }}>{t("trackingNumber")}:{item.transactionData.item_id}</Text>
...
function OnGoing(props) {
  ...
  useEffect(() => {
    ...
    axios.get('https://europe-west1-mysehelling-backend.cloudfunctions.net/api/user/onGoing')
      .then(result => {
        if (isMounted === true) {
          setData(result.data);
          props.setActivityLoader({isLoading:false})
        }
      }).catch(err => {
        console.log(err);
      });
...
export default connect(mapStateToProp, mspDispatchToProp())(OnGoing);
